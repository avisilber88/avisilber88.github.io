...
if (!canvas){
	   ...
	   console.error(idArray);
		...
		for (var i = 0; i < data.length; i++){
			if ((data[i][50]=="S1")&&(data[i][39]=="3")){
			groupThenNameArray.push([data[i][2], data[i][0], data[i][3], data[i][4], data[i][26]]);
			}
		}
		console.log(data);
...
for (var i = 0; i < groupThenNameArray.length; i++) {
	...
	// rowsInExport.sort();
	groupThenNameArray.unshift(["Grade", "ID", "First Name", "Last Name", "Counselor"]);
