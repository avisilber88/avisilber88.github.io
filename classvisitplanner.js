$(document).ready(function () {
	...
	//THE IMPORTANT PLACE WE SEND INFO FOR THE SPREADSHEET IS "THIS IS WHERE WE GET THE INFO FOR THE SPREADSHEET"
...
console.log(data[0][0].toString());
	 ...
	 canvasSectionColumn =38; // was 43 and it's how they made the course departments before. Now we are making it the rooms: 38
		}
		// alert ("Canvas!");
...
for (var i = 0; i < groupThenNameArray.length; i++) {
	...
	rowsInExport.unshift(["Group", "Names"]);
		...
		breakoutfilename=prompt ("What would you like to call your exported spreadsheet csv?")+".csv";
 console.warn(groupThenNameArray.toString());
		let csvContent = "data:text/csv;charset=utf-8," + rowsInExport.map(e => e.join(",")).join("\n").replace(/"/g,"");
		var encodedUri = encodeURI(csvContent);
var link = document.createElement("a");
...
for (var i = 0; i < groupThenNameArray.length; i++) {
			...
			tempNameHere=groupThenNameArray[i][1];
			groupThenNameArray[i][1] = tempNameHere.split(', ').slice(-1).join(' ')+ " " + tempNameHere.split(', ').slice(0, -1).join(' ');			
           rowsInExport.push([groupThenNameArray[i][0], groupThenNameArray[i][1],groupThenNameArray[i][2],groupThenNameArray[i][3]]);//, groupThenNameArray[i][1]]); //addAssignmentOption(dataArray[2][i].substring(0, dataArray[2][i].indexOf("MAX")));
        }
	// rowsInExport.sort();
	rowsInExport.unshift(["Group", "Names"]);
	   ...
	   console.error(idArray);
		...
		for (var i = 0; i < data.length; i++){
			for (var j = 0; j < selectedSections.length; j++){
				...
				if ((data[i][50]==selectedSemester)&&(data[i][38].includes(selectedSections[j]))){ //THIS IS WHERE WE GET THE INFO FOR THE SPREADSHEET
...
for (var i = 0; i < groupThenNameArray.length; i++) {
           ...
           rowsInExport.push([groupThenNameArray[i][0], groupThenNameArray[i][2], groupThenNameArray[i][3]]);//, groupThenNameArray[i][1]]); //addAssignmentOption(dataArray[2][i].substring(0, dataArray[2][i].indexOf("MAX")));
        }
	
	// rowsInExport.sort();
	console.log(groupThenNameArray.toString());
	
	groupThenNameArray.sort(comparePeriod);
	groupThenNameArray.sort(compareRoomNumber);
	groupThenNameArray.unshift(["Grade", "Teacher", "ID", "Pd", "Room", "Class", "First Name", "Last Name"]);
 console.warn(groupThenNameArray);
	const whatIsYourRoom = prompt ("What is your room #?");
 	console.log("Optimal Itinerary:", getFewestClassesWithProximity(groupThenNameArray, getFloor(whatIsYourRoom)));	

		breakoutfilename=prompt ("What would you like to call exported spreadsheet csv?")+".csv";
 console.warn(groupThenNameArray.toString());
		let csvContent = "data:text/csv;charset=utf-8," + getFewestClassesWithProximity(groupThenNameArray, getFloor(whatIsYourRoom)).map(e => e.join(",")).join("\n").replace(/"/g,"");
		var encodedUri = encodeURI(csvContent);
var link = document.createElement("a");
link.setAttribute("href", encodedUri);
link.setAttribute("download", breakoutfilename);
...
link.click();
    ...
    var checkSections = function (data) {
		document.getElementById('selectionsBox').innerHTML = "<table id = 'pdTable'><tr><td><div class = 'picksection' style = 'width:auto'>Which periods are you planning to visit during?<br><td><tr></div></table>";//<select id = 'sectionSelect' name = 'sectionSelect' style = 'font-size:large; width:auto; max-width:300px;'> <option value = 'cation1'> cation1 </option> <option value = 'cation2'> cation2 </option><option value = 'cation3'> cation3 </option><option value = 'cation4'> cation4 </option> </select>  <button type ='button' id ='submitSection' style='font-size: xx-large'>Submit</button></div>";
			...
			// for (var i = 0; i < data[44].length; i++) {
				...
				// console.log(sectionArray);
				// //if (sectionOption.includes(data[44][i].substring(0,3))==false){
				// addSectionOption(data[44][i].substring(0,3));
				// //}
			// }
	
        	for (var i = 0; i < sectionArray.length; i++) {
				// alert(sectionArray.toString());
				console.log(chosenSoFar.toString().includes(sectionArray[i]));
				console.log(sectionArray[i].toString());
					if (chosenSoFar.toString().includes(sectionArray[i])==false){
						chosenSoFar.push(sectionArray[i].toString());
						//addSectionOption(sectionArray[i]);
					}
					
				//chosenSoFar.push(sectionArray[i].toString());//sectionArray[i].substring(0,3));
			}
			for (var l = 0; l < chosenSoFar.length; l++){
			document.getElementById('pdTable').innerHTML = document.getElementById('pdTable').innerHTML + "<tr><td><label for='subscribe"+chosenSoFar[l]+"'>Period "+chosenSoFar[l]+"</label><input type='checkbox' id='subscribe"+chosenSoFar[l]+"' name='newsletter"+chosenSoFar[l]+"' value='subscribed'></td></tr>";
			
			//document.getElementById('subscribe'+chosenSoFar[l]).checked=false;
			
			}
				document.getElementById('pdTable').innerHTML = document.getElementById('pdTable').innerHTML + "<tr><td><button type ='button' id ='submitSection' style='font-size: xx-large'>Submit</button></td></tr>";
			
			"<button type ='button' id ='submitSection' style='font-size: xx-large'>Submit</button></div>";
			//document.getElementById('selectionsBox').innerHTML = document.getElementById('selectionsBox').innerHTML + "</table>";
			...
			for (var j = 0; j < chosenSoFar.length; j++){
			console.log(document.getElementById('subscribe5').checked);

			if (document.getElementById('subscribe'+chosenSoFar[j]).checked) {
				selectedSections.push(chosenSoFar[j]);
				console.log(chosenSoFar[j]);
			}
			// }
			}
			console.log(selectedSections.toString());
            // alert(document.getElementById("assignmentSelect").options[document.getElementById("assignmentSelect").selectedIndex].innerHTML);
            // }
...
//submitSection.click();
	  ...
	  const getFloor = (roomStr) => {
	const isAString = /[a-zA-Z]/.test(String(roomStr));
    const match = String(roomStr).match(/\d/);
    return isAString ? parseInt(-2) : parseInt(match[0]) ;
  };
function getFewestClassesWithProximity(givenData, myFloor) {

  // 1. UPDATED MAPPING (Swapped Room and Class based on your update)
  const COL = {
    GRADE: 0,
    TEACHER: 1,
    ID: 2,
    ROOM: 4,   // Switched: Now index 3 is the Room Number (e.g., 124)
    LAST: 7,
    FIRST: 6,
    CLASS: 5,  // Switched: Now index 6 is the Class Name (e.g., Ceramic Sculpture)
    PD: 3
  };
const cleanData = givenData.slice(1);
  ...
  while (true) {
    ...
    cleanData.forEach(row => {
      const studentId = String(row[COL.ID]).trim();
      if (seenStudents.has(studentId) || !studentId) return;
      
      const room = String(row[COL.ROOM]).trim();
      const upperRoom = room.toUpperCase();
      if (upperRoom.includes("GYM") || upperRoom.includes("PORT") || upperRoom.includes("FIELD")) return;

      const key = `${room}|${row[COL.PD]}`;
      if (!classMap[key]) classMap[key] = [];
      classMap[key].push(row);
    });

    for (const [key, students] of Object.entries(classMap)) {
      let score = students.length; 
      if (getFloor(students[0][COL.ROOM]) === myFloor) score += 3;
      if (Math.abs(getFloor(students[0][COL.ROOM])-myFloor)>1) score -= 3;
		console.log("P2 means " + getFloor("P2") + " and Main Gym Means " + getFloor("Main Gym") + " and 307 means " + getFloor(307));
      if (score > maxScore) {
        maxScore = score;
        bestClassKey = key;
        bestRoster = students;
  ...
  // Header as requested
  const output2D = [["Grade", "Teacher Last", "Teacher First", "ID", "Pd", "Room", "Class", "First", "Last"]];

  stops.forEach(stop => {
    stop.roster.forEach(student => {
      output2D.push([
        student[COL.GRADE],
        student[COL.TEACHER],
        student[COL.ID],
...
// EXECUTION
// const results = getGlobalOptimalItinerary(yourArrayData, 1); // Replace 1 with your floor
// console.log("OPTIMIZED STOPS:");
// console.table(results.itinerary);

// if (results.missing.length > 0) {
  // console.warn("STUDENTS IN STRING ROOMS (Not Visited):");
  // console.table(results.missing);
// }

  // return finalItinerary;
// }; //end of closest distance 

// console.log("Optimal Itinerary:", getFewestClassesWithProximity(students, 100));	
...
//Step 2B: Grouping Style
    ...
    var generatePasses = function () {
		...
		document.getElementById('selectionsBox').innerHTML = '<div class="col-xs-8 col-xs-offset-2" >		<font color= "Black">Hopefully that was helpful! If you would like to start again, refresh!	</div>'
...
// if (largestScore>20)
    ...
    function compareSecondColumn(a, f) {
        if (a[1] === f[1]) {
            return 0;
        } else {
            return (a[1] < f[1]) ? -1 : 1;
	...
	function compareRoomNumber(a, f) {
        if (a[4] === f[4]) {
            return 0;
        } else {
            return (a[4] < f[4]) ? -1 : 1;
