...
var previousJumped=false;
var previousJumpedDown=false;
var specificNotes = false;
var specificCorrectChord = [];
var specificActiveNotes = [];
var currentStep = 0;
var score = 1;
...
function showNotes(arrayOfNotes, positionNumber) {
...
// console.error(octaveAdder+" "+octaveSetting);
    // if (pickedInversion != 0) {
        // octaveAdder = octaveSetting + 12; ;
    // }
	
	let octaveAdder=octaveSetting;
// console.error("invert? "+pickedInversion);
    if (positionNumber > 0) {
        // console.warn(((activeNoteSequence.length + 1) * 2) + " " + arrayOfNotes.length + " " + arrayOfNotes[positionNumber]);
        if ((document.getElementById("upDown").selected)) {
            if (((activeNoteSequence.length) * 2) < arrayOfNotes.length) { //we are underhalf way
                if (arrayOfNotes[positionNumber] < arrayOfNotes[positionNumber - 1]) {
                    // console.warn(arrayOfNotes.toString());
                    octaveAdder = octaveAdder + 12;
					previousJumped=true;
                }
				else if (previousJumped){
				previousJumped=false;
				 if (arrayOfNotes[positionNumber] < (arrayOfNotes[positionNumber - 1]+12)) {
                // console.warn(arrayOfNotes.toString());
					octaveAdder = octaveAdder + 12;
					previousJumped=true;
				 }
				}
            } else { //we are over half way
			// console.error("halfway");
                if (arrayOfNotes[positionNumber] > arrayOfNotes[positionNumber - 1]) {
                    octaveAdder = octaveSetting;
					// console.error('up');
					previousJumpedDown=true;
                }
				else{
					octaveAdder = octaveSetting+12;
						// console.error('down');
					if (previousJumpedDown){
						octaveAdder = octaveSetting;
						// previousJumpedDown=false;
						if (arrayOfNotes[positionNumber] > (arrayOfNotes[positionNumber - 1])){
						octaveAdder=octaveSetting;
						}
					}
		...
		// console.error(arrayOfNotes);

    makeAndShowANote(arrayOfNotes[positionNumber] + octaveAdder, '2', "referenceNote");
	// specificCorrectChord.push(arrayOfNotes[positionNumber] + octaveAdder);
    ...
    // if ((pickedInversion != 0)) {
        ...
        // if (pickedInversion <= i) {
            ...
            // correctNoteSequence[positionNumber] = arrayOfNotes[positionNumber] + octaveAdder - 12;
            // makeAndShowANote((arrayOfNotes[positionNumber] + octaveAdder - 12) + '', '2', "referenceNote");
        // } else {
            // makeAndShowANote(arrayOfNotes[positionNumber] + octaveAdder, '2', "referenceNote");
            // correctNoteSequence[positionNumber] = arrayOfNotes[positionNumber] + octaveAdder;
        // }
    // }

    // makeAndShowANote(arrayOfNotes[positionNumber] + octaveAdder, '2', "referenceNote");
	// console.error(arrayOfNotes[positionNumber] + octaveAdder+0);
	// let temparray= JSON.parse(JSON.stringify(arrayOfNotes));
	// console.error(temparray.toString());
	// console.error(correctNoteSequence.toString());
    // correctNoteSequence[positionNumber] = temparray[positionNumber] + octaveAdder;
	// let temparray= JSON.stringify(arrayOfNotes)
    // var notes = [
    // ['c', '#', '4'],
    // ['e', 'b', '5'],
    // ['g', '', '5'],
...
function makeSpecificCorrect() {
	previousJumped=false;
	previousJumpedDown=false;
	specificCorrectChord=[];
	let temparray= JSON.parse(JSON.stringify(correctNoteSequence));
    // let noteStr = noteArray[arrayOfNotes[0]+24][1];
    // let partOne = noteStr.slice(0, 1) + '';
    // let partTwo = noteStr.slice(+2) + '';
    // let partThree = noteStr.slice(1, 2) + '';
...
// console.error(octaveAdder+" "+octaveSetting);
	...
	arrayOfNotes= JSON.parse(JSON.stringify(correctNoteSequence));
	for (let itt=0; itt<temparray.length; itt++){
		// console.error(itt);
		positionNumber=itt;
	octaveAdder=octaveSetting;
// console.error("invert? "+pickedInversion);
    if (positionNumber > 0) {
        ...
        if ((document.getElementById("upDown").selected)) {
			// alert('jumped0');
             if (((positionNumber) * 2) < arrayOfNotes.length) { //we are underhalf way
                // alert('jumped2');
				if (arrayOfNotes[positionNumber] < arrayOfNotes[positionNumber - 1]) {
                    // console.warn(arrayOfNotes.toString());
                    octaveAdder = octaveAdder + 12;
			...
			// console.error("halfway");
                if (arrayOfNotes[positionNumber] > arrayOfNotes[positionNumber - 1]) {
					...
					octaveAdder = octaveSetting+12;
						// console.error('down '+previousJumpedDown);
					if (previousJumpedDown){
						octaveAdder = octaveSetting;
						// previousJumpedDown=false;
						if (arrayOfNotes[positionNumber] > (arrayOfNotes[positionNumber - 1])){
    ...
    // makeAndShowANote(arrayOfNotes[positionNumber] + octaveAdder, '2', "referenceNote");
	// console.error(arrayOfNotes[positionNumber] + octaveAdder);
	let tempNum=Number(JSON.stringify(arrayOfNotes[positionNumber] + octaveAdder+24));
	// console.error(tempNum);
	
	specificCorrectChord.push(tempNum);
	}
    // noteStr = noteArray[arrayOfNotes[0]+24][1];
    // partOne = noteStr.slice(0, 1) + '';
    // partTwo = noteStr.slice(+2) + '';
    ...
    // visibleNoteGroups.shift();
    // }, 5000);
// specificCorrectChord=temparray;
previousJumped=false;
...
function noteOnListener(note, velocity) {
    ...
    case 1:
        ...
        specificActiveNotes.push(note);
        rightAnswer();
        try {
			//console.error(correctNoteSequence.length);
			
           if (correctNoteSequence.length!=activeNoteSequence.length)
			   showNotes(correctNoteSequence, activeNoteSequence.length);
        } catch (error) {console.error(error)}
        ...
        if (activeNoteSequence.length == correctNoteSequence.length) {
            ...
            for (let index = 0; index < activeNoteSequence.length; index++) {
                if (activeNoteSequence[index] != correctNoteSequence[index]) {
                    match = false;
                    octaveAdder = octaveSetting;

                    showNotes(correctNoteSequence, 0);
                    break;
                }
            }
            if ((specificNotes)&&(match)) {
				// specificCorrectChord=[]
				makeSpecificCorrect();
				// console.error(specificCorrectChord);
				
				// console.error(specificActiveNotes);
                for (let index2 = 0; index2 < specificActiveNotes.length; index2++) {
                    if (specificActiveNotes[index2] != specificCorrectChord[index2]) {
                        match = false;
						alert ("correct.. but wrong octave");
                        octaveAdder = octaveSetting;
...
function countChordTypesSelected() {
    ...
    if (document.getElementById("majS").selected) { //checking to see if ________ is checked
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Major Scale:</strong><br>W<br>W<br>h<br>W<br>W<br>W<br>h<br> </td>";
        ezmajon = true;
    ...
    if (document.getElementById("minS").selected) { //checking to see if ________ is checked
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Minor Scale:</strong> <br>W<br>h<br>W<br>W<br>h<br>W<br>W<br></td>";
    ...
    if (document.getElementById("harmS").selected) { //checking to see if ________ is checked
        ...
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Harmonic Minor Scale:</strong> <br>W<br>h<br>W<br>W<br>h<br>3 semitones<br>h<br></td>";
    ...
    if (document.getElementById("pentaS").selected) { //checking to see if ________ is checked
        ...
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Pentatonic Scale:</strong> <br>W<br>W<br>3 semitones<br>W<br></td>";

        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("boogieS").selected) { //checking to see if ________ is document.getElementById"jln".selected

        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Boogie-Woogie Scale: </strong> <br>4 semitones<br>3 semitones<br>W<br>h<br></td>";

        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("bluesS").selected) { //checking to see if ________ is document.getElementById"jln".selected
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Pentatonic Blues Scale:</strong> <br>3 semitones<br>W<br>h<br>h<br>3 semitones<br>W<br></td>";
        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("dorianS").selected) { //checking to see if ________ is document.getElementById"jln".selected
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Dorian Scale: </strong> <br>W<br>h<br>W<br>W<br>W<br>h<br>W<br></td>";
        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("phrygianS").selected) { //checking to see if ________ is document.getElementById"jln".selected
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Phrygian Scale: </strong> <br>h<br>W<br>W<br>W<br>h<br>W<br>W<br></td>";
        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("lydianS").selected) { //checking to see if ________ is document.getElementById"jln".selected
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Lydian Scale:</strong> <br>W<br>W<br>W<br>h<br>W<br>W<br>h<br></td>";
        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("mixolydianS").selected) { //checking to see if ________ is document.getElementById"jln".selected
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Mixolydian Scale:</strong> <br>W<br>W<br>h<br>W<br>W<br>h<br>W<br></td>";
        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
    if (document.getElementById("locrianS").selected) { //checking to see if ________ is document.getElementById"jln".selected
        totalselected++;
        insertHTML = insertHTML + "<td style = 'vertical-align:top'><strong>Locrian Scale:</strong> <br>h<br>W<br>W<br>h<br>W<br>W<br>W<br></td>";
        if ((totalselected - subtractor) == 4) {
            insertHTML = insertHTML + "</tr><tr style = 'border: 1px solid black'>";
        }
    }
...
$('#sheetButton').click(function () {
    if (presentationType == "sheet") {
        ...
        document.getElementById("resetButton").style.left = '50vw';
        ...
        document.getElementById("resetButton").style.left = '60vw';
    }
    if (score > 4) {
        score = score - 3;
