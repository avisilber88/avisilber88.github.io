...
let volume = 0.0;
function nextNote() {
  const secondsPerStep = (60.0 / currentBPM)/stepsPerBeat;
...
function scheduleNote(stepNumber, nextTime, volume) {
	// console.error(scheduledNoteCounter+"/"+foreverNoteCounter);
  // push the note on the queue, even if we're not playing.
  if (scheduledNoteCounter<foreverNoteCounter){
	  scheduledNoteCounter++;
  notesInQueue.push({note: stepNumber, nextTime: nextTime, volume: volume});//this is pushing an array including a note with the value stepNumber, and a time with a value time
  }
  ...
  //if (pads[3].querySelectorAll('button')[stepNumber].getAttribute('aria-checked') === 'true') {
   ...
   if (notesInQueue.length>0){
		...
		playSample(audioCtx, dtmf, nextTime, volume);
...
function scheduler() {
  ...
  while (nextStepTime < audioCtx.currentTime + scheduleAheadTime ) {
	  console.log("testa");
	  scheduleNote(currentStepPosition, nextStepTime, volume);
      nextNote();
	  console.log("testb");
  }
...
//var fs = require('fs')
// const midiSomething = new Midi();
//const midiSomething2 = Midi.fromUrl("https://www.nwhsaob.com/Midi/practicesetone.mid");
async function getMidiFile(filepath) {
  const obj = await Midi.fromUrl(filepath);
  //const arrayBuffer = await response.arrayBuffer();
  console.log(obj);
  // // ! A callback has been added here as a second param for Safari only !
  // const audioBuffer = await audioContext.decodeAudioData(arrayBuffer, function() {return});
  //MidiParser.Uint8(arrayBuffer);
    ...
    let lastNote = 0;
    let currentNote = 0;
    var midiFormatType = 1;
    // var midiFormatType2 = obj.formatType;
	//alert(midiFormatType);
    // midiFormatType=1;
    if (midiFormatType == 1) {
        let newTempo = obj.header.tempos[0].bpm;
		// alert (newTempo);
        let tempNewTempo = newTempo;
        let noteContainingTrack = 0;
		// alert(obj.tracks[2].name);
        for (var i = 0; i < (obj.tracks.length); i++) {
            try {
                // for (var j = 0; j < (obj.tracks.length); j++) {
			
				//alert(obj.tracks[i].name);
					if (obj.tracks[i].notes.length >0){
						//console.log (i+" "+j+" "+obj.track[i].event[j].data);
						trackList.push(obj.tracks[i].name);
					console.log(i);

                    // if ((obj.tracks[i].event[j].type == 9) && (!trackPicked)) {
...
document.getElementById('selectionsBox').innerHTML = "<div class = 'pickassignment'><select id = 'assignmentSelectGroup' name = 'assignmentSelectGroup' style = 'font-size:large'> <optgroup id = 'assignmentSelect' label='Practice by Dylan'><option value = 'cation1'> cation1 </option> <option value = 'cation2'> cation2 </option><option value = 'cation3'> cation3 </option><option value = 'cation4'> cation4 </option> </optgroup><optgroup id = 'otherMusic' label = 'Other Selections'><option value = 'window'>Waving Through a Window</option><option value = 'hercules'>Go the Distance</option><option value = 'satisfied'>Satisfied</option></optgroup></select>  </div>";
    ...
    let currentNote = 0;
            ...
            // if (true){
				//console.log("hi bob");
            for (var i = 0; i < trackList.length; i++) {
                if (document.getElementById("assignmentSelectGroup").options[document.getElementById("assignmentSelectGroup").selectedIndex].innerHTML == (trackList[i])) {
        ...
        for (var i = 0; i < (obj.tracks[noteContainingTrack].notes.length); i++) {
            // console.log(obj.track[obj.track.length-1].event[0].data);
            // console.log(obj.timeDivision);
          var importTicksPerBeat = 1;

            // console.log(obj.track[2].event[i].deltaTime);
            // console.log(obj.track[noteContainingTrack].event[i].type);
          ticksOfThisNote = ticksOfThisNote + obj.tracks[noteContainingTrack].notes[i].time-beatsOfLastNote; //add the time of the message to the building message.
            beatsOfLastNote = obj.tracks[noteContainingTrack].notes[i].time;
			  // if ((obj.track[noteContainingTrack].event[i].type == 9) && (obj.track[noteContainingTrack].event[i].channel == channels)) { //if note on
				...
				console.log(obj.tracks[noteContainingTrack].notes[i].name+ " "+obj.tracks[noteContainingTrack].notes[i].time);
			  lastNote = currentNote + 0;
                ...
                currentNote = obj.tracks[noteContainingTrack].notes[i].midi;
                ticksOfThisNote = 0;
            }
...
function makeAndShowANote(noteArrayNum, theNoteLength, voiceType) {
	...
	if (tickContext.tickables.length>0){
	tickContext.tickables.pop();
	}
    ...
    if (notationMode == "movingNotesMode") {
		...
		console.error("tick context size "+tickContext.tickables.length);
        // visibleNoteGroups.push(group);
...
function allRightAnswer() {
    try {
        for (var i = 0; i < visibleReferenceNoteGroups.length; i=0) {
            ...
            group.style.transform = `translate(${x-900}px, -0px)`;
        }
    } catch (error) {}
}
...
function checkDistance() {
    ...
    try {
		console.log("IT IS!!!: " +visibleReferenceNoteGroups.length);
...
function readyAnswer() {
    ...
    try {
		...
		if (playerMode){
			console.log("fake match called");
		fakeMatch();
		console.log("fake match called");
		}
    } catch (error) {
		console.error ("ERRORRRRRRRR IN READYANSWER!!!");
		console.log(error);
	}
}
...
nextvolume = 1.0; // setting it to 10%
   ...
   // noteStr = noteStr.slice(0, 1) + noteStr.slice(+2) + noteStr.slice(1, 2);
	setupSample("https://www.nwhsaob.com/Midi/samplesthree/"+noteStr+".mp3")
  .then((sample) => {
  ...
  // });
    let audiophone ="https://www.nwhsaob.com/Midi/samplesthree/"+noteStr;
    // console.warn(noteStr);
...
function draw() { //THIS IS FOR DRAWING BUT IT IS STILL SORT OF IMPORTANT!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  while (notesInQueue.length && notesInQueue[0].time < currentTime) {
	  console.log("NOTES TO DRAW: "+notesInQueue.length);
console.log("NOTES TIME: "+notesInQueue[0].time + " of " + currentTime);
...
function noteOnListener(note, velocity) {
	...
	let something = getMidiFile("https://www.nwhsaob.com/Midi/practicesetone.mid");// 
...
async function setCountdownMessage (countdowntext){
	console.log("the countdown message");
document.querySelector('#countdown').innerText = countdowntext;
}
...
$('#defaultMidiButton').click(function () {

let something = getMidiFile("https://www.nwhsaob.com/Midi/Study1.mid");// 
