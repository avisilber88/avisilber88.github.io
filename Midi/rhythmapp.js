...
alertify ("Welcome to Rhythm Sense!");
	sequenceOn.click();
	turnAccompanimentOn();
...
// alert ('hi');
	...
	catch(error){alert (visibleReferenceNoteGroups.toString());}
}
...
kickSequenceArray.push(["kick", thisRandomKickTime/4 , 0]);
...
for (var stepnum = 0; stepnum < 16; stepnum++){
	...
	if (kickStepArray[stepnum].classList.contains("selectedstep")){
		...
		tempSequenceArray.push(["kick", thiskickLength+0, 0]);
...
if (firstkicksounded){
	if (kickOnOne){
		tempSequenceArray.push(["kick", thiskickLength+0+kicksToAddToEnd, 0]);
	}
	else {
		tempSequenceArray.push(["kick",  thiskickLength+0, 0]);
		tempSequenceArray.unshift(["rest", kicksToAddToEnd+0, 0]);
	}
}
let didYouKickIt=(JSON.stringify(tempSequenceArray) == JSON.stringify(kickSequenceCopy));

console.warn("temp sequence "+tempSequenceArray.toString()+ " kick sequence "+ kickSequenceCopy);
if (didYouKickIt){
totalScore++;
...
newQuestionTime=true;
resetStepElements();
alertify ("Nice Job! You got that one!")
}
}
...
$('#rhythmFreestyleButton').click(function () {
	...
	document.getElementById('rhythmFreestyleButton').innerHTML="Turn Freestyle Mode On";
	}
	else{
	rhythmFreestyle=true;
	document.getElementById('rhythmFreestyleButton').innerHTML="Turn Freestyle Mode Off";
	}
	sequenceStarted=false;
	newQuestionTime=true;
});
