...
//	answerString=round(answerString,3)+"";
	...
	var setupAnswers=function(carbonNumber, organicQuality){ // the array coming in has [balancedFormula, catNum, anNum];

			// var moles = n1*Math.pow(10, m1)/mW;
			// var molar = moles/(n2*Math.pow(10,m2));
			
var wrongOrganicQuality1=1;
var wrongOrganicQuality2=1;
var wrongOrganicQuality3=1;

var wrongOrganicQuality1=organicQuality+1;
if (wrongOrganicQuality1>4){
	wrongOrganicQuality1=wrongOrganicQuality1-4;
}
var wrongOrganicQuality2=organicQuality+2;
if (wrongOrganicQuality2>4){
	wrongOrganicQuality2=wrongOrganicQuality2-4;
}
var wrongOrganicQuality3=organicQuality+3;
if (wrongOrganicQuality3>4){
	wrongOrganicQuality3=wrongOrganicQuality3-4;
}


	var randomunitnum5=(Math.floor(Math.random()*2)+1);
wrongCompound1="";
wrongCompound2="";
wrongCompound3="";
	
	switch (randomunitnum5){ 
			case 1:  //wrong type of thing
	//alert ("done");
			wrongCompound1 = getWrongOrganicFormula1(carbonNumber, wrongOrganicQuality1)
			wrongCompound2 = getWrongOrganicFormula1(carbonNumber, wrongOrganicQuality2); //gohere813
			wrongCompound3 = getWrongOrganicFormula1(carbonNumber, wrongOrganicQuality3);  //gohere813
			break;	
			case 2: //wrong # of carbons.
			
				//alert ("done3");
			wrongCompound1 = getOrganicFormula	((Math.floor(Math.random()*10)+1), (Math.floor(Math.random()*4)+1)); //gohere813
			wrongCompound2 = getOrganicFormula ((Math.floor(Math.random()*10)+1), (Math.floor(Math.random()*4)+1)); //gohere813
			wrongCompound3 = getOrganicFormula ((Math.floor(Math.random()*10)+1), (Math.floor(Math.random()*4)+1)); //gohere813
			break;
		}
...
wrongAnswer1=wrongCompound1;
wrongAnswer2=wrongCompound2;
wrongAnswer3=wrongCompound3;
answer=getOrganicFormula(carbonNumber, organicQuality);
		var answers = [];
		answers[0]=answer;
		answers[1]=wrongCompound1;
		answers[2]=wrongCompound2;
		answers[3]=wrongCompound3;
...
var getAlkane =function(carbonx) {
...
var getWrongOrganicFormula1 = function (carbonx, quality, ansNum){
var organicFormula = "blank";
var carbony=carbonx;
if (carbony==1){
	carbony="";
	...
	case 3:
		//alert ("4");
		organicFormula = "C" + (""+carbony).sub()+"H"+(""+(carbonx*2+1)).sub();
		break;
	case 4:
		//alert ("1");
		organicFormula = "C" + (""+carbony).sub()+"H"+(""+(carbonx*2+2)).sub()+"OH";
...
var getWrongOrganicFormula2 = function (carbonx, quality, ansNum){
var organicFormula = "blank";
var carbony=carbonx;
if (carbony==1){
	carbony="";
	...
	case 2:
		//alert ("3");
		organicFormula = "C" + (""+carbony).sub()+"H"+(""+(carbonx*2+ansNum)).sub();
		break;
	case 3:
		//alert ("4");
		organicFormula = "C" + (""+carbony).sub()+"H"+(""+(carbonx*2+ansNum)).sub();
	...
	case 4:
		//alert ("1");
		organicFormula = "C" + (""+carbony).sub()+"H"+(""+(carbonx*2+ansNum)).sub()+"OH";
...
return polyCheck;
	...
	var resetQuestion=function(){
		...
		var randomunitnum3=(Math.floor(Math.random()*10)+1);
		var carbonNumber = randomunitnum3;
		//alert (cationCharge);
		//var cationArray = getRandomCation(cationCharge);
		//carbonNumber=1;
		if (carbonNumber==1){
		var randomunitnum2=(Math.floor(Math.random()*2))*3+1;
				}
		else{
		var randomunitnum2=(Math.floor(Math.random()*4)+1);
			}
		var organicQuality = randomunitnum2;
...
if (score>19){
	document.getElementById("num1").innerHTML=("what is the chemical formula for "+getOrganicPrefix(carbonNumber)+getOrganicSuffix(organicQuality)+"?");		
}
else {
...
//$('#num1').text("What is the molarity of a "+formulaName+" solution do we get when we mix " +toOurExponential(sigFigs(coeff1, 3))+ units1 " of "+ formulaName + " in " +  toOurExponential(sigFigs(coeff1, 3)) + units2 " of //water? (" +formulaName+" has a molar mass of " + molarMass+" grams/mole)");
			...
			setupAnswers(carbonNumber, organicQuality); 
	};
