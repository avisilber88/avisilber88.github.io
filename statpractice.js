...
var roundTypeName;
	var resetQuestion = function () {
		...
		numOfNums=Math.floor(Math.random()*5)+3;
		//console.log("The number of numbers is "+numOfNums);
		for (i = 0; i < numOfNums; i++) {
			let newNum=Math.floor(Math.random()*10);
			arrayOfNums.push(newNum);
			stringOfNums=stringOfNums+newNum+", ";
			sumOfNums=sumOfNums+newNum;
		}
		
		averageNum=sumOfNums*1.0/numOfNums;
		...
		case 1:
			if (score > 19) {
				document.getElementById("num1").innerHTML = "during a pilot run of vaccine vials, the quality control team inspected 20 vials per day over "+numOfNums+" days, for a total of "+(numOfNums*20)+" vials. The number of defects recorded each day was as follows: "+stringOfNums+" each value was for a different day. Using the formula below, calculate the standard deviation of the defects data. Record your answer to "+numberOfSigFigsToCount+" "+roundTypeName+" below.";
