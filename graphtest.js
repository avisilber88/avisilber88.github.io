...
// }
	...
	let expectedY=Math.round((xyDotArray[0]-xValuesOfLine[i])*tempSlope+yValuesOfLine[i]*10)/10;
...
var data1 = [];
var data2 = [{x: randomX, y: randomY}];
for (var j = 0; j<xValues.length; j++){
	data1.push({x: xValues[j], y: yValues[j]});//);
}
...
let tempMessage= checkSaturation(tempArray, xValues, yValues);
		...
		//alert (questionType);
			...
			if (score > 19) {
							document.getElementById("num1").innerHTML = "what is the status of the red +?";
