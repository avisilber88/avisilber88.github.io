...
for (var i = 0; i < groupThenNameArray.length; i++) {
	...
	groupThenNameArray.unshift(["Grade", "Teacher Last", "Teacher First", "ID", "Pd", "Class", "Room", "First Name", "Last Name"]);
		...
		breakoutfilename=prompt ("What would you like to call exported spreadsheet csv?")+".csv";
 console.warn(groupThenNameArray.toString());
		let csvContent = "data:text/csv;charset=utf-8," + groupThenNameArray.map(e => e.join(",")).join("\n").replace(/"/g,"");
		var encodedUri = encodeURI(csvContent);
var link = document.createElement("a");
link.setAttribute("href", encodedUri);
link.setAttribute("download", breakoutfilename);
...
link.click();
    ...
    var checkSections = function (data) {
		document.getElementById('selectionsBox').innerHTML = "<div class = 'picksection' style = 'width:auto'>What department do you want to export a list based on?<br><select id = 'sectionSelect' name = 'sectionSelect' style = 'font-size:large; width:auto; max-width:300px;'> <option value = 'cation1'> cation1 </option> <option value = 'cation2'> cation2 </option><option value = 'cation3'> cation3 </option><option value = 'cation4'> cation4 </option> </select>  <button type ='button' id ='submitSection' style='font-size: xx-large'>Submit</button></div>";
  				$('#sectionSelect').empty();
        	...
        	for (var i = 0; i < sectionArray.length; i++) {
				// alert(sectionArray.toString());
				console.log(chosenSoFar.toString().includes(sectionArray[i].substring(0,3)));
					if (chosenSoFar.toString().includes(sectionArray[i].substring(0,3))==false){
						addSectionOption(sectionArray[i].substring(0,3));
					}
				chosenSoFar.push(sectionArray[i].substring(0,3).toString());//sectionArray[i].substring(0,3));
			}
