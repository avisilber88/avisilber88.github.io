...
// if (exponency.substring(0,1)==="+"){
	...
	resetQuestionDecimals(Number(molar));
	};
	...
	var resetQuestionDecimals = function (input) {
		...
		setupAnswersDecimals(finalNum, 0);
	};
...
firstpH=Math.floor(Math.random()*14)+1;
secondpH=Math.floor(Math.random()*14)+1;

let localAnswer=Math.pow(10, secondpH-firstpH);
if (((pWhat=="pH")&&(acidOrBass=="acidic"))||((pWhat=="pOH")&&(acidOrBass=="basic"))){
	// console.log(pWhat+acidOrBass);
	if (firstpH>secondpH){
		let placeHolderpH=secondpH;
		secondpH=firstpH;
		firstpH=placeHolderpH;
	}
	localAnswer=Math.pow(10, secondpH-firstpH);
}
else{
	// console.log("nope" +pWhat+acidOrBass);
	if (firstpH<secondpH){
		let placeHolderpH=secondpH;
		secondpH=firstpH;
		firstpH=placeHolderpH;
	...
	localAnswer=Math.pow(10, firstpH-secondpH);
}

if (score>19){
document.getElementById("num1").innerHTML = "a solution with a "+pWhat+" of "+firstpH +" is how many times more " + acidOrBass + " than a solution with a "+pWhat+" of " + secondpH;
}
else {
